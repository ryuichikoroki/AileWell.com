...
| 設立 | 2024年5月 |
| 所在地（登記） | 〒252-0016 神奈川県座間市西栗原2-1-51 ※**現在サイト掲載中**（商標登録の早期審査要件対応） |
| 所在地（旧／実営業所） | 〒243-0342 神奈川県海老名市中央1-16-31 A&NIビル06 ※`TMP_ADDRESS_SWAP` マーカー内にコメントアウトで温存 |
...
| 2026/04/20 | 商標登録の早期審査要件対応：サイト掲載住所を登記住所（座間市西栗原）に一時切替。旧住所（海老名）は `TMP_ADDRESS_SWAP` マーカー内にコメントアウトで温存（about / privacy / access の JA・EN 全6ファイル + access の駅情報・地図） |
